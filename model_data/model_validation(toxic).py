...
# 모델 폴더 경로를 지정.
MODEL_PATH = r"D:/PJ/Advanced_PJ/Antidote/model_data/model/toxic_model_v3"
# 검증 데이터셋 파일 경로를 지정.
# VAL_DATA_PATH = r"D:/PJ/Advanced_PJ/Law_Data/dataset/Validation_Set(label)/Validation(cross_encoder)2.csv"
# 전체 학습 데이터 오탐 진행시
VAL_DATA_PATH = r"D:/PJ/Advanced_PJ/Antidote/model_data/Law_Data/dataset/Training_Set(label)/학습데이터(toxic)/Toxic_clause_extraction.csv"
...
# 시각화용 함수 정의
def visualize_attention(clause_text, model, tokenizer, layer_idx=11):
    model.config.output_attentions = True
    inputs = tokenizer(clause_text, return_tensors="pt", truncation=True, max_length=128).to(device)
...
# --- 결과 리포트 출력 ---
print("\n" + "="*50)
print("📊 [검증 결과 요약 - Toxic Detection]")
...
print("\n📝 [상세 분류 리포트]")
print(classification_report(true_labels, pred_labels, target_names=['정상(0)', '독소(1)']))
...
plt.title('Confusion Matrix - Toxic Clause Detection')
...
if not incorrect_cases.empty:
    ...
    # 오답 리스트를 엑셀로 저장
    incorrect_cases.to_csv("D:/PJ/Advanced_PJ/Antidote/model_data/Law_Data/false_data/toxic_error_analysis_1.csv", index=False, encoding="utf-8-sig")
